...
import { LayoutDashboard, BookOpen, FileText, CheckSquare, Moon, Sun, CalendarDays, LogOut } from 'lucide-react';
...
function AnimatedBackground({ darkMode }: { darkMode: boolean }) {
  return (
    <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
      ...
      <motion.div animate={{ x:[0,60,-30,0], y:[0,-80,40,0], scale:[1,1.2,0.9,1] }} transition={{ duration:18, repeat:Infinity, ease:'easeInOut' }}
      ...
      <motion.div animate={{ x:[0,-50,70,0], y:[0,60,-40,0], scale:[1,0.85,1.15,1] }} transition={{ duration:22, repeat:Infinity, ease:'easeInOut', delay:3 }}
        className={`absolute top-1/3 -right-40 w-[450px] h-[450px] rounded-full blur-3xl opacity-25 ${darkMode?'bg-green-900':'bg-green-200'}`} />
      <motion.div animate={{ x:[0,40,-60,0], y:[0,-50,70,0], scale:[1,1.1,0.95,1] }} transition={{ duration:26, repeat:Infinity, ease:'easeInOut', delay:6 }}
  ...
  return (
    <div className="min-h-screen transition-colors duration-700">
      ...
      <motion.header initial={{ y:-100 }} animate={{ y:0 }} transition={{ type:'spring', stiffness:100 }}
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <motion.div initial={{ opacity:0, x:-20 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.2 }}>
            <h1 className={`text-xl font-bold tracking-tight ${darkMode?'text-white':'text-gray-900'}`}>Student Dashboard</h1>
          </motion.div>
      ...
      <motion.nav initial={{ y:100 }} animate={{ y:0 }} transition={{ type:'spring', stiffness:100, delay:0.2 }}
        className={`fixed bottom-0 left-0 right-0 z-50 md:hidden border-t ${glass}`}>
            ...
            return (
              <Link key={item.path} to={item.path}>
                <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.3+index*0.08 }}
                  ...
                  <div className={`p-2.5 rounded-2xl transition-all ${isActive ? darkMode?'bg-emerald-500/30 text-emerald-300 border border-emerald-400/30':'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30' : darkMode?'text-gray-400':'text-gray-500'}`}>
                    <Icon className="w-5 h-5" />
                  </div>
                  <span className={`text-[10px] font-semibold transition-all ${isActive?darkMode?'text-emerald-300':'text-emerald-600':darkMode?'text-gray-500':'text-gray-400'}`}>{item.label}</span>
                </motion.div>
              </Link>
            );
          })}
      ...
      <motion.nav initial={{ x:-100 }} animate={{ x:0 }} transition={{ type:'spring', stiffness:100, delay:0.2 }}
        className={`hidden md:flex flex-col fixed left-0 top-20 bottom-0 w-72 p-6 border-r ${glass}`}>
        <div className="space-y-1">
          {navItems.map((item, index) => {
            ...
            return (
              <Link key={item.path} to={item.path}>
                <motion.div initial={{ opacity:0, x:-20 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.3+index*0.08 }}
                  whileHover={{ x:6 }} whileTap={{ scale:0.97 }}
                  className={`relative flex items-center gap-4 px-5 py-4 rounded-2xl transition-all font-bold tracking-wide ${isActive ? darkMode?'bg-emerald-500/20 text-emerald-300 border border-emerald-400/25 shadow-lg':'bg-emerald-500/15 text-emerald-700 border border-emerald-300/40 shadow-lg shadow-emerald-500/10' : darkMode?'text-gray-400 hover:bg-white/5 hover:text-gray-200':'text-gray-500 hover:bg-white/40 hover:text-gray-700'}`}>
                  {isActive && (
                    <motion.div layoutId="activeTabDesktop"
                      className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 rounded-r-full ${darkMode?'bg-emerald-400':'bg-emerald-500'}`}
