...
export default function TodoList() {
  ...
  useEffect(() => {
    const data = storageService.getTodos();
    setTodos(data);
  }, []);
  ...
  const toggleTodo = (id: string) => {
    ...
    // Trigger confetti if completing a todo
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="space-y-3">
          <Reorder.Group
            ...
            onReorder={(newOrder) => {
              // Only update if we're viewing 'all' to preserve order
              if (filter === 'all') {
                saveTodos(newOrder);
              }
            }}
  ...
  const cardContent = (
    <>
      ...
      >
        {canReorder && (
          <motion.div
            className="cursor-grab active:cursor-grabbing text-gray-400 dark:text-gray-600"
        ...
        <motion.button
          ...
          className={`flex-shrink-0 ${
            todo.completed ? 'text-green-500' : 'text-gray-400 dark:text-gray-600'
          }`}
        >
          {todo.completed ? (
  ...
  if (canReorder) {
    return (
      <Reorder.Item
        ...
        className="flex items-center gap-4 p-4 rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 cursor-pointer"
