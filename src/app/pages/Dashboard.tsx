...
// Subjects to skip when importing (sport, admin, etc.)
const SKIP_KEYWORDS = ['WSREC','BADMINTON','CARE:','ADMIN'];

// Color map so the same subject always gets the same colour
...
// ── ICS PARSER ───────────────────────────────────────────────────────
// Converts UTC datetimes to AEST (UTC+11 for AEDT, UTC+10 for AEST)
// Deduplicates by (dayOfWeek + subject + startTime) so Week A/B merges cleanly
function parseIcsToClasses(text: string): ClassPeriod[] {
  const blocks = text.split('BEGIN:VEVENT').slice(1);
  // key = "day|subject|startTime" → ClassPeriod
  ...
  for (const block of blocks) {
    ...
    // Extract teacher from DESCRIPTION field (format: "Teacher: Name\nPeriod: N")
    let teacher = '';
    const teacherMatch = desc.replace(/\\n/g, '\n').match(/Teacher:\s*([^\n]+)/);
    if (teacherMatch) teacher = teacherMatch[1].trim();

    // Parse UTC datetime string e.g. "20260127T210000Z"
    ...
    // The ICS spans Jan–Apr 2026 which is all AEDT (UTC+11)
    ...
    const dayOfWeek = localStart.getUTCDay(); // 0=Sun, 1=Mon...
    // Skip weekends
    ...
    // Dedup key: same subject + same day + same start = one slot
    const dedupKey = `${dayOfWeek}|${summary}|${startTime}`;
    if (seen.has(dedupKey)) continue;

    const color = getSubjectColor(summary, colorIndex++);
  ...
  return Array.from(seen.values())
    .sort((a,b) => a.dayOfWeek - b.dayOfWeek || parseTime(a.startTime) - parseTime(b.startTime));
}
...
export default function Dashboard() {
  ...
  // Refresh every 30s to update now/next badges
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!next) return;
    const update = () => setCountdown(getTimeUntil(next.startTime, next.dayOfWeek));
    update();
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="flex items-start justify-center gap-1.5 mb-8">
                  <DigitBlock value={pad2(countdown.h)} label="hrs" dark={darkMode} />
                  <Colon dark={darkMode} />
                  <DigitBlock value={pad2(countdown.m)} label="min" dark={darkMode} />
                  <Colon dark={darkMode} />
                  <DigitBlock value={pad2(countdown.s)} label="sec" dark={darkMode} />
                </div>
...
function IcsModal({ dark, onClose, onImport }: {
  dark:boolean; onClose:()=>void; onImport:(p:ClassPeriod[])=>Promise<void>;
}) {
    ...
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
      ...
      <motion.div initial={{ scale:0.96, opacity:0, y:12 }} animate={{ scale:1, opacity:1, y:0 }} exit={{ scale:0.96, opacity:0, y:12 }}
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-1 leading-relaxed">
          Export from Sentral / Google Calendar / Outlook as <strong className="font-semibold text-gray-700 dark:text-gray-300">.ics</strong>, open in a text editor, copy everything, paste below.
        </p>
        <p className="text-xs text-emerald-600 dark:text-emerald-400 mb-4">
          ✓ Times auto-converted to AEST &nbsp;·&nbsp; ✓ Week A/B duplicates removed &nbsp;·&nbsp; ✓ Weekends skipped
        </p>
